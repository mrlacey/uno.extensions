...
internal static class MauiInterop
{
	...
	public static bool TryGetStyle(Style winUIStyle, out Microsoft.Maui.Controls.Style? style)
	{
		...
		if (winUIStyle.TargetType is null || !TryGetMapping(winUIStyle.TargetType, out var targetType) || targetType is null)
		{
			return false;
		}
		else if (!winUIStyle.Setters.Any() && winUIStyle.BasedOn is null)
		{
			return false;
		}
	...
	private static List<IWinUIToMauiStyleHandler> GetHandlers(Type targetType) =>
		_styleHandlers.Where(x => x.TargetType == targetType || x.TargetType.IsAssignableFrom(targetType)).ToList();
